    ...
    <div class="container">
        <div class="border border-bg-dark mt-5 mb-5 pl-5 pr-5 shadow p-3 mb-5  bg-white rounded">
            <div class="bg-dark text-light" >
                ...
                <p class ="text-center mt-4 pb-5"> Publié le : <?=date("d/m/Y", strtotime($postById['date_post'])); ?>  </p>
            </div>
        ...
        <div class="mb-5 shadow p-3 mb-5">
            ...
            <?php foreach ($commentsById as $commentById) :?>
                <div class="text-center border border-bg-dark bg-light mt-2 pb-2">
                    <p class="mt-2 ml-2 mb-0"><strong><?=$commentById['pseudo_comment'] ?></strong> à écrit le <?=date("d/m/Y", strtotime($commentById['date_comment'])); ?></p>
                    ...
                    <form method="POST" action="?page=deletecomment&id=<?=$commentById['id_comment']?>" onSubmit="return confirm('voulez-vous vraiment supprimer ce commentaire ?');">
                            <button class="btn btn-danger mt-2" type="submit">Supprimer le commentaire</button>
                    </form>
                </div>    
            <?php endforeach; ?>  
        </div>
